...
void JSph::InitVars(){
  ...
  UsePartialSlip = false; // SHABA partial slip
  UseDEM=false;  //(DEM)
  ...
  PhaseCount=0;
  memset(&PhaseBound,0,sizeof(StPhaseBound));
...
void JSph::LoadConfigParameters(const JXml *xml){
  ...
  if(TBoundary==BC_MDBC){ //<vs_mddbc_ini>
    ...
    switch(eparms.GetValueInt("SlipMode",true,1)){
	  ...
	  case 4:  SlipMode = SLIP_NoSlip; UsePartialSlip = true; break;
...
void JSph::LoadConfigCommands(const JCfgRun *cfg){
  ...
  if(cfg->TBoundary){
    ...
    switch(cfg->TBoundary){
	  ...
	  case 4:  TBoundary = BC_MDBC;  SlipMode = SLIP_NoSlip; UsePartialSlip = true; break;
...
void JSph::LoadCaseConfig(const JCfgRun *cfg){
  ...
  */                                                //<vs_chroono>
  ...
  if(xml.GetNodeSimple("case.execution.special.nnphases",true)){
      //For beta version disable some options
	  ...
	  //if(TStep==STEP_Verlet)Run_Exceptioon("Multiphase formulations are not supported with Verlet in Beta version.");
...
void JSph::ConfigConstants(bool simulate2d){
  ...
  if(MultiPhase)ConfigConstantsMP(); //<vs_non-Newtonian>
...
void JSph::InitMultiPhase(const JXml *sxml, std::string xmlpath) {
  ...
  if(PhaseCount<2)Run_Exceptioon("The number of phases is invalid.");
  ...
  for (unsigned c = 0; ele;c++) {
    ...
    PhaseArray[c].Gamma = sxml->ReadElementFloat(ele, "gamma", "value",true);//printf("%s \n", PhaseArray[c].Gamma ? "Gamma before true" : "Gamma before false");
    if (!PhaseArray[c].Gamma)PhaseArray[c].Gamma = Gamma;//printf("%s \n", PhaseArray[c].Gamma ? "Gamma true" : "Gamma false");
    ...
    PhaseCte[c].reg_NN = sxml->ReadElementFloat(ele, "reg_NN", "value");
    PhaseCte[c].reg_strain = sxml->ReadElementFloat(ele, "reg_strain", "value");       
    PhaseCte[c].coh = sxml->ReadElementFloat(ele, "cohesion", "value", true);
    PhaseCte[c].phi = sxml->ReadElementFloat(ele, "phi", "value", true);
    //PhaseCte[c].yieldcriterion = sxml->ReadElementUnsigned(ele, "yieldcriterion", "value");
    PhaseCte[c].phasetype = sxml->ReadElementUnsigned(ele, "phasetype", "value");   //Sediment or fluid(NN) 
...
void JSph::ConfigConstantsMP(){
  ...
  if(Cs0_present){
    ...
    for (unsigned c = 0; c < PhaseCount; c++) {
      PhaseArray[c].CteB=float(Cs0*Cs0*PhaseArray[c].rho / PhaseArray[c].Gamma);
      PhaseArray[c].mass=float(PhaseArray[c].rho*(Simulate2D? Dp*Dp: Dp*Dp*Dp));
    }       
  }
  CoefDtMin*=0.0000001f;  //GFCheck: Are you sure?.
}
  ...
  ,tfloat4 *velrhop,float *auxNN)const
{
  ...
  //-first option.
  //for (unsigned cp = 0; cp < PhaseCount; cp++) {
  //  const unsigned ini = PhaseCte[cp].idbegin;
  //  const unsigned n = PhaseCte[cp].count;
  //  const float rhop = PhaseArray[cp].rho_ph;
  //  for (unsigned p = 0; p < n; p++) {
  //      velrhop[p + ini].w = rhop;
  //  }
  //}
  ...
  for (unsigned p = 0; p < np; p++) {
    ...
    if (CODE_IsFluid(cod)) {
      ...
      //auxNN[p] = PhaseCte[cp].visco;
    }
    //Keep BC as default
    //else if(CODE_IsNotFluid(cod)){
    //  unsigned cp = CODE_GetTypeValue(cod);           
    //  velrhop[p].w = PhaseBound.rho_ph;
    //}
  }
}
